...
class WebConfigRangeSetting(DotnetRangeSetting):
    ...
    def decode_option_json(self, data, path=None):
        ...
        if not wc:
            raise SettingRuntimeException("WebConfig dict for path:setting {}:{} was not found in describe data".format(path, self.name))
        if not isinstance(wc, dict):
            raise SettingRuntimeException('Describe WebConfig data {} must have its value be a dict. '
                                         'It is currently {}.'.format(q(self.name), wc.__class__.__name__))
        if len(wc) < 0:
            raise SettingRuntimeException("WebConfig dict for path:setting {}:{} was found but had no key values".format(path, self.name))
